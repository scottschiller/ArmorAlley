...
const Tutorial = () => {
  ...
  function updateHighlightControls(step, active) {
    ...
    const query = highlightControls
      .map((type) => `[data-for="${type}"]`)
      .join(',');

    oControls
      .querySelectorAll(query)
      .forEach((node) =>
        utils.css.addOrRemove(node, active, css.getUserAttention)
      );
  }
  ...
  function animate() {
    ...
    if (
      data.frameCount % data.animateModulus === 0 &&
      data.step !== null &&
      config.steps[data.step]
    ) {
      config.steps[data.step].animate();
    }
  ...
  function initTutorial() {
    ...
    addStep({
      ...
      animate() {
        ...
        const chopper = game.players.local,
    ...
    addStep({
      ...
      animate() {
        return (
          game.players.local.data.parachutes >=
          game.players.local.data.maxParachutes
        );
      },
    ...
    addStep({
      ...
      animate() {
        return (
          countSides(TYPES.missileLauncher).enemy === 0 &&
          countSides(TYPES.smartMissile).enemy === 0
        );
      },
    ...
    addStep({
      ...
      animate() {
        ...
        return !t.isEnemy && !t.dead && t.energy === t.energyMax;
      },
    ...
    addStep({
      ...
      animate() {
        ...
        return (
          !turrets[1].data.isEnemy ||
          turrets[1].data.dead ||
          !turrets[2].data.isEnemy ||
          turrets[2].data.dead
        );
      },
