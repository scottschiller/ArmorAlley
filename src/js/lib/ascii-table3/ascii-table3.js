...
class AsciiTable3 {
  ...
  static padStart(str, maxLength, fillStr = ' ') {
    if (strlen(str) >= maxLength) {
      ...
      if (partArray.length > 0) {
        ...
        printable = fillStr
          .repeat(maxLength - endPrintLen - strlen(printable))
          .concat(printable);
    ...
    if (strlen(str) >= maxLength) {
      ...
      if (partArray.length > 1) {
        ...
        printable = printable.concat(
          fillStr.repeat(maxLength - initialPrintLen - strlen(printable))
        );
  ...
  static align(direction, value, len, pad = ' ') {
    ...
    if (direction == AlignmentEnum.RIGHT) {
      ...
      return AsciiTable3.padEnd(
        AsciiTable3.padStart(
          strValue,
          strlen(strValue) + Math.floor((len - strlen(strValue)) / 2),
          pad
        ),
        len,
        pad
    ...
    if (strlen(str) > maxSize) {
      ...
      for (const [nonPrintable, printable] of partition(str)) {
        const text = Array.from(
          printable.substring(0, maxSize - SUFIX.length).concat(SUFIX)
        ).slice(0, maxSize - length);
  ...
  setStyle(name) {
    ...
    this.style = foundStyle
      ? foundStyle
      : this.styles.find((style) => style.name == 'ramac');
  ...
  setAlign(idx, direction) {
    if (this.dataAlign) {
      ...
      this.dataAlign.concat(
        AsciiTable3.arrayFill(idx - this.dataAlign.length, AlignmentEnum.AUTO)
      );
    } else {
      // create array
  ...
  getAligns() {
    if (!this.dataAlign) {
      ...
      if (this.getRows().length > 0) {
        // defaults to auto
        this.dataAlign = AsciiTable3.arrayFill(
  ...
  setWrapped(idx, wrap = true) {
    if (this.wrapping) {
      // add new array elements if needed
      this.wrapping.concat(
        AsciiTable3.arrayFill(idx - this.wrapping.length, false)
      );
    } else {
  ...
  transpose() {
    ...
    if (this.getHeading().length == 0 && this.getRows().length == 0)
      return this;
    ...
    for (var row = 0; row < newMatrix.length; row++) {
      ...
      for (var col = dataStartCol; col < newMatrix[0].length; col++) {
        ...
        newMatrix[row][col] = this.getCell(
          col + Math.abs(dataStartCol - 1),
          row + 1
        );
      }
    }
  ...
  getWrappedRows(row) {
    ...
    for (var col = 0; col < row.length; col++) {
      ...
      if (this.getWidth(col + 1) && this.isWrapped(col + 1)) {
        wrappedRow[col] = AsciiTable3.wordWrap(
          cell,
          this.getWidth(col + 1) - this.getCellMargin() * 2
        ).split('\n');
  ...
  getHeadingRowTruncated(posStyle, colsWidth, row) {
    ...
    for (var col = 0; col < row.length; col++) {
      ...
      const cellAligned = AsciiTable3.align(
        this.getHeadingAlign(),
        cell,
        colsWidth[col] - this.getCellMargin() * 2
      );

      result +=
        ''.padStart(this.getCellMargin()) +
        AsciiTable3.truncateString(
          cellAligned,
          colsWidth[col] - this.getCellMargin() * 2
        ) +
        ''.padStart(this.getCellMargin());
  ...
  toString() {
    ...
    if (this.getTitle().length > 0) {
      ...
      result +=
        style.borders.top.left +
        ''.padStart(maxWidth, style.borders.top.center) +
        style.borders.top.right +
        '\n';
